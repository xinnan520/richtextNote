package com.xinnan.richtextnote.richtextnote.db;
...
public class NoteDao {
        ...
        try {
            if (groupId > 0){
                sql = "select * from db_note where n_group_id =" + groupId +
                        "order by n_create_time asc";
            } else {
                sql = "select * from db_note order by n_create_time asc" ;
            }
    ...
    public List<Note> queryNoteByKeyword(String keyword) {
        ...
        List<Note> noteList = new ArrayList<>();
        Note note ;
        String sql = "select * from db_note where n_title like '%"+keyword+"%' or n_content like '%"+keyword+"%'";
        Cursor cursor = null;
